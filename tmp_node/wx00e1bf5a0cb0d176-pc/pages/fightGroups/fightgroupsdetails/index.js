var t = getApp(), a = (require("../../../components/utils/util.js"), require("../../shop/ShopUtil.js"), 
require("../../../common.js"));
...
Page({
    ...
    onLoad: function(e) {
        ...
        this.setData({
            queryparams: e,
            pinTuanInventory: e.storenum,
            language: t.globalData.language
        }), s.isLogin(), a.setPopupFromShare();
    },
    isLogin: function() {
        setTimeout(function() {
        ...
        t.sendRequest({
            ...
            success: function(e) {
                if (e.success) {
                    ...
                    var s = e.data.pintuan_user_num, n = e.data.pintuan_num - s, r = e.data.pintuan_user_list, i = [];
                    if (n > 0) for (var u = 0; u < n; u++) i.push({
                        user_img: ""
                    });
                    ...
                    for (var o = 0; o < r.length; o++) t.globalData.WebUserID == r[o].UserID && a.setData({
                        show: !0
                    });
                    var d = "", l = e.data.pintuan_remain_time;
                    l < 86400 ? a.data.pintuan_remain_time = setInterval(function() {
                        var t = --l, e = Math.floor(t / 60 / 60 % 24, 10), s = Math.floor(t / 60 % 60, 10), n = Math.floor(t % 60, 10);
                        e < 10 && (e = "0" + e), s < 10 && (s = "0" + s), n < 10 && (n = "0" + n);
                        var r = e + ":" + s + ":" + n;
                        a.setData({
                            leftTime: r
                        }), l <= 0 && clearInterval(a.data.pintuan_remain_time), l <= 0 && (l = 0);
                    }, 1e3) : a.data.pintuan_end_time = setInterval(function() {
                        var t = --l, e = Math.floor(t / 60 / 60 / 24, 10), s = Math.floor(t / 60 / 60 % 24, 10), n = Math.floor(t / 60 % 60, 10), r = Math.floor(t % 60, 10);
                        e < 10 && (s = "0" + e), s < 10 && (s = "0" + s), n < 10 && (n = "0" + n), r < 10 && (r = "0" + r);
                        var i = e + "天" + s + "时" + n + "分" + r + "秒";
                        a.setData({
                            leftTime: i
                    ...
                    success: function(t) {
                        if (t.success) if (0 == t.data.attrkeys.length) a.setData({
                            ...
                            for (n = 0; n < t.data.attrkeys.length; n++) {
                                ...
                                for (u = 0; u < t.data.attrkeys[n].attrVals.length; u++) t.data.attrkeys[n].attrVals[u].isSelect = !1, 
                                t.data.attrkeys[n].attrVals[u].canSelect = !0;
                            }
                            for (var e = [], s = [], n = 0; n < t.data.skus.length; n++) {
                                var r = t.data.skus[n];
                                0 != r.ProductQuantity ? e.push(r) : s.push(r);
                            }
                            ...
                            for (n = 0; n < t.data.attrkeys.length; n++) for (var u = 0; u < t.data.attrkeys[n].attrVals.length; u++) -1 == a.data.hasInventoryPathArr.indexOf(t.data.attrkeys[n].attrVals[u].AttrValID) && (t.data.attrkeys[n].attrVals[u].canSelect = !1, 
                            a.setData({
                                noKuCunId: t.data.attrkeys[n].attrVals[u].AttrValID
                            }));
                            a.setData({
    ...
    selectSku: function(t) {
        var a = t.currentTarget.dataset.attrvalid, e = t.currentTarget.dataset.attrkeyid, s = t.currentTarget.dataset.attrvalname, n = t.currentTarget.dataset.hasimage, r = t.currentTarget.dataset.canselect, i = this.minThreeNum();
        if (0 != r) {
            if (this.data.buyCount > i && this.setData({
                buyCount: i
            }), 1 == n) for (h = 0; h < this.data.attrkeys.length; h++) if (1 == this.data.attrkeys[h].hasImg && this.data.attrkeys[h].AttrKeyID == e) for (o = 0; o < this.data.attrkeys[h].attrVals.length; o++) this.data.attrkeys[h].attrVals[o].AttrValID == a && this.setData({
    ...
    arrCheck: function(t) {
        for (var a = {}, e = 0; e < t.length; e++) {
            for (var s = t[e], n = 0, r = 0; r < t.length; r++) t[r] == s && (n++, t[r] = -1);
            -1 != s && (a[s] = n);
        }
        return a;
    },
    ...
    selectHalf: function(t) {
        ...
        for (r = 0; r < a.length; r++) for (n = 0; n < a[r].length; n++) -1 == this.data.selectValArr.indexOf(a[r][n]) && (console.log(a[r][n]), 
        e.push(a[r][n]));
        for (s = 0; s < this.data.attrkeys.length; s++) for (r = 0; r < this.data.attrkeys[s].attrVals.length; r++) -1 !== e.indexOf(this.data.attrkeys[s].attrVals[r].AttrValID) && (this.data.attrkeys[s].attrVals[r].canSelect = !1);
        for (var a = [], e = [], s = 0; s < this.data.noSkuInventoryArr.length; s++) 1 == this.isContained(this.data.noSkuInventoryArr[s].Path, t) && a.push(this.data.noSkuInventoryArr[s].Path);
        for (r = 0; r < a.length; r++) for (var n = 0; n < a[r].length; n++) -1 == this.data.selectValArr.indexOf(a[r][n]) && e.push(a[r][n]);
        for (s = 0; s < this.data.attrkeys.length; s++) for (var r = 0; r < this.data.attrkeys[s].attrVals.length; r++) -1 !== e.indexOf(this.data.attrkeys[s].attrVals[r].AttrValID) && (this.data.attrkeys[s].attrVals[r].canSelect = !1);
    },
    decrease: function() {
    ...
    sureBuy: function() {
        0 === this.data.attrkeys.length || 0 != this.data.isSelectAllAttr ? 0 !== this.data.attrkeys.length && this.data.buyCount > this.data.pinTuanInventory ? wx.showModal({
            ...
            url: "../square/index?pintuan_id=" + this.data.pintuan_id + "&pintuan_group_id=" + this.data.pintuan_group_id + "&pintuan_product_id=" + this.data.pintuan_product_id + "&pintuan_product_num=" + this.data.buyCount + "&sku_id=" + this.data.skuid + "&attrImg=" + this.data.attrImg
        })) : wx.showModal({
            title: "提示",
            content: "请选择规格",
            showCancel: !1
    ...
    onShareAppMessage: function(a) {
        ...
        var s = "/pages/fightGroups/fightgroupsdetails/index?fromShare=1&pintuan_group_id=" + e.data.pintuan_group_id;
        return {
    ...
    goPinTuanProDetail: function() {
        ...
        t.setData({
            ...
            url: "../productdetail/index?pintuan_id=" + this.data.pintuan_id + "&id=" + this.data.pintuan_product_id + "&img=" + t.data.product.img_src + "&price=" + t.data.product.sale_price + "&storenum=" + t.data.product.remain_stock + "&proid=" + t.data.product.product_id + "&buycount=" + t.data.product.buy_limit_num
        });
    },
