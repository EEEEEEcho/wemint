...
Page({
    ...
    JumpUrlByType: function JumpUrlByType(typeId, urls) {
        switch (typeId) {
          case 10:
    ...
    getHomeData: function getHomeData(res) {
        var that = this;
    ...
    getHomeProductData: function getHomeProductData(pageIndex, addPageIndex) {
        var that = this;
    ...
    CheckVersionNumber: function CheckVersionNumber(that) {
    ...
    DownloadTopcis: function DownloadTopcis(that) {
        //保存数据
        wx.request({
    ...
    HomeTopicData: function HomeTopicData(that) {
        wx.getStorage({
            key: "topiclist",
            success: function success(res) {
                that.setData({
    ...
    bindConfirmSearchInput: function bindConfirmSearchInput(e) {
        var keyword = e.detail.value;
        if (keyword.length > 0) {
            wx.setStorage({
                key: "keyword",
    ...
    bindSearchAction: function bindSearchAction(e) {
    ...
    onSkuClick: function onSkuClick(e) {
        ...
        this.data.CurrentProduct.Skus.forEach(function(item, index, array) {
            var found = true;
            for (var i = 0; i < selSku.length; i++) {
                if (selSku[i] == undefined || item.SkuId.indexOf("_" + selSku[i].ValueId) == -1) found = false;
            }
