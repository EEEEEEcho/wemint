...
Page({
   ...
   */
    ...
    selectList: function selectList(e) {
        var that = this;
        var dataset = e.currentTarget.dataset, tempselectskuId = that.data.SelectskuId;
    ...
    selectShop: function selectShop(e) {
        var that = this, strskuId = [], totalmoney = 0, idx = e.currentTarget.dataset.idx, shopstatus = !that.data.shopSelectAll[idx].shopSelect, tempshopcart = that.data.ShopCarts[idx], ids = that.data.SelectskuId, allstatus = false;
    ...
    SwitchEdite: function SwitchEdite() {
        var that = this;
    ...
    countNum: function countNum(e) {
        var that = this;
        var dataset = e.currentTarget.dataset, item = that.data.ShopCarts[dataset.sidx][dataset.idx], oldquantity = parseInt(item.Count);
    ...
    bindblurNum: function bindblurNum(e) {
        var that = this;
        var dataset = e.currentTarget.dataset, item = that.data.ShopCarts[dataset.sidx][dataset.idx], oldquantity = parseInt(item.Count);
    ...
    DelCarts: function DelCarts(e) {
        var that = this;
        var delSkuid = e.currentTarget.dataset.skuid;
        var delSkuidStr = that.data.SelectskuId;
        //获取要删除的skuId；
                var totalmoney = 0;
        if (delSkuid != "") {
            app.getOpenId(function(openid) {
    ...
    SettlementShopCart: function SettlementShopCart() {
        var that = this;
        ...
        //获取要删除的skuId；
            ...
            //获取所选商品的CartItemId
                        var cartidArr = [], selectSkus = that.data.SelectskuId;
    ...
    ChangeQuantiy: function ChangeQuantiy(that, quantity, skuId) {
        app.getOpenId(function(openid) {
            wx.request({
