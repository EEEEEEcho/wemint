...
var cityRegionIds = new Array();

//已加载的城市ID
var areadata = new Array();

var areaRegionIds = new Array();
...
Page({
    ...
    bindShipToTap: function bindShipToTap(e) {
        var shipTo = e.detail.value;
        var that = this;
        that.data.shipTo = shipTo;
    },
    bindCellPhoneTap: function bindCellPhoneTap(e) {
        var cellPhone = e.detail.value;
        var that = this;
        that.data.cellPhone = cellPhone;
    },
    bindFullAddressTap: function bindFullAddressTap(e) {
        p = 0, c = 0, d = 0;
        this.setAreaData();
        this.setData({
            showDistpicker: true
    ...
    bindAddressTap: function bindAddressTap(e) {
        var address = e.detail.value;
        var that = this;
        that.data.address = address;
    },
    bindSaveTapTap: function bindSaveTapTap(e) {
        var that = this;
    ...
    getEditAddressData: function getEditAddressData(res) {
    ...
    changeArea: function changeArea(e) {
        ...
        p = e.detail.value[0];
        c = e.detail.value[1];
        if (e.detail.value.length > 2) d = e.detail.value[2]; else d = 0;
        s = 0;
    ...
    showDistpicker: function showDistpicker() {
        this.setData({
            showDistpicker: true
        });
    },
    distpickerCancel: function distpickerCancel() {
        this.setData({
            showDistpicker: false
        });
    },
    distpickerSure: function distpickerSure() {
        console.log("确定省：" + p + "确定市：" + c + "确定区：" + d);
        var fullAddress = this.data.provinceName[p] + " " + this.data.cityName[c] + " " + this.data.districtName[d];
        //+ this.data.streetName[s];
                var regionId;
    ...
    getRegions: function getRegions(regionId, depth, a, s) {
        var that = this;
        var hasData = true;
        if (depth == 3) {
            //如果未包含则获取
    ...
    setProvinceCityData: function setProvinceCityData(data, p, c, a, s) {
        var that = this;
        if (data != null) {
            procitydata = data;
        }
    ...
    getItemIndex: function getItemIndex(arr, item) {
        var i = arr.length;
        var index = -1;
        while (i--) {
            if (arr[i] === item) {
    ...
    setAreaDataShow: function setAreaDataShow(data, parentRegionId, a, s) {
        var that = this;
        if (data != null) {
            currAreaData = data;
    ...
    setStreetData: function setStreetData(data, parentRegionId, a, s) {
        var that = this;
        if (data != null) {
    ...
    setAreaData: function setAreaData(p, c, d, s) {
        var that = this;
        var p = p || 0;
        // provinceSelIndex
                var c = c || 0;
