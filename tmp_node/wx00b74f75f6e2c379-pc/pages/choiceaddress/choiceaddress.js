...
Page({
    ...
    bindDeleteAddressTap: function bindDeleteAddressTap(e) {
        var that = this;
    ...
    getAddressResultData: function getAddressResultData(res) {
        var that = this;
    ...
    bindEditAddressTap: function bindEditAddressTap(e) {
        console.log(e);
        var addressData = e.currentTarget.dataset.addressdata;
        var that = this;
        if (this.data.IsCheck == 0) {
            wx.redirectTo({
                url: "../editaddress/editaddress?extra=" + JSON.stringify(addressData) + "&title=" + "编辑收货地址" + "&Source=choiceaddress&" + "productsku=" + that.data.ProductSku + "&cartItemIds=" + that.data.cartItemIds + "&buyamount=" + that.data.BuyAmount + "&frompage=" + that.data.FromPage + "&countdownid=" + that.data.CountdownId
            });
        }
    },
    ...
    //点击添加收货地址
    onAddShippingAddress: function onAddShippingAddress(e) {
        var that = this;
        wx.showModal({
            title: "提示",
            content: "是否使用微信收货地址",
    ...
    onAddressCheck: function onAddressCheck(e) {
        var that = this;
        console.log(e);
