...
Page({
    ...
    loadMyList: function(a) {
        ...
        if (e > i) {
            var o = Math.ceil(i / s.data.mypagesize) + 1, r = "";
            s.data.sortcol && (r = "&sortcol=" + s.data.sortcol + "&sort=" + s.data.sort), s.data.queryparams.use && (s.data.use = s.data.queryparams.use), 
            t.sendRequest({
                url: "/index.php?c=Front/WxApp/ServiceApi&a=getUserCouponList&page=" + o + "&pagesize=" + s.data.mypagesize + r,
                method: "GET",
                success: function(a) {
                    if (a.success) {
